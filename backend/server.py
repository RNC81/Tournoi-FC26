...
from fastapi import FastAPI, APIRouter, HTTPException, Body, Depends, status, Request, Query
...
from pydantic import BaseModel, Field, ConfigDict, field_validator, StringConstraints, ValidationError
...
async def complete_groups_and_draw_knockout(
    ...
    if tournament.format == "2v2" and remix:
        ...
        # Capturer les paires des équipes qualifiées AVANT le remix
        for team_name in qualified_entities:
            parts = team_name.split(' + ')
            if len(parts) == 2:
                previous_teams_sets.append(set(parts))
        
        # Extraire tous les joueurs individuels des équipes qualifiées
        individual_pool = []
        ...
        # Tenter de créer de nouvelles paires sans collision
        attempts = 0
        valid_shuffle = False
        new_teams = []
        ...
        while attempts < 100 and not valid_shuffle:
            ...
            for i in range(0, len(individual_pool), 2):
                if i+1 < len(individual_pool):
                    ...
                    # Vérifier si cette paire existe déjà dans les équipes qualifiées
        ...
        # Si aucun shuffle valide après 100 tentatives, utiliser le dernier
...
async def generate_next_round(tournament_id: str, remix: bool = Query(True, description="Remixer les équipes (True) ou garder les mêmes (False)")):
    ...
    # Remix des équipes gagnantes (ou pas selon le paramètre)
    if remix:
        # Remix activé : mélanger les joueurs
        individual_pool = []
        ...
        # Pas de remix : garder les équipes telles quelles
        new_teams = winning_teams
    ...
    # Petite finale (3ème place) - toujours remixée
...
@api_router.post("/test-security", summary="Test de résilience aux injections NoSQL", response_description="Résultat du test")
async def test_security_injection(payload: dict = Body(...)):
    """
    Simule une tentative d'injection NoSQL via le champ 'username'.
    Essaie de créer un objet UserCreate avec le payload fourni.
    Retourne le succès ou l'erreur de validation.
    """
    try:
        # On force la validation Pydantic manuellement pour voir si elle bloque l'input
        user = UserCreate(**payload)
        return {
            "status": "DANGER",
            "message": "L'injection a RÉUSSI ! Le validateur n'a pas bloqué l'input.",
            "validated_data": user.model_dump()
        }
    except ValidationError as e:
         return {
            "status": "SECURE",
            "message": "L'injection a ÉCHOUÉ. Le validateur Pydantic a bloqué l'input.",
            "error_details": str(e)
        }
    except Exception as e:
        return {
